...
from mintpy import plate_motion, smallbaselineApp
...
for i, (key, value) in enumerate(STEPS.items()):
    stepmsg += f'{i+1}.  {key:20s}{str(value)} \n'
...
def cmd_line_parse(iargs=None):
    ...
    if (inps.json_file) and (os.path.exists(inps.json_file) is False):
        print(f'Parameter JSON file {inps.json_file} does not exist')
        parser.print_help()
        sys.exit(1)
    if len(inps.dosteps) == 0:
        print(f'No steps specified: {inps.dosteps}')
        sys.exit(1)
    return inps
...
def get_template(jobj):
    ...
    print(f'Use template for regular pairs: {jobj.template}')
    ...
    print(f'Use template for ionospheric pairs: {jobj.templateIon}')
...
def run_radar2geo_inputs(jobj, inps, lalo=None, ram=None):
    ...
    print(f'Go to the directory: {in_dir}')
    os.chdir(in_dir)
    ...
    print(f'Go back to the directory: {jobj.proc_home}')
...
def run_ifgram_inversion(template, ifstack, mask, weight='var', cluster=None, n_worker=None, ram=None):
    ...
    print(f'Multiprocessing: cluster: {cluster}  num_workers: {n_worker}')
...
def run_generate_mask(jobj, coherence=None, threshold=None, outfile=None):
    proc_home = os.path.expanduser(jobj.proc_home)
    if not coherence:
        coherence = f'{proc_home}/{jobj.tcoh_file}'
    if not threshold:
        threshold =    f'{jobj.tcoh_threshold}'
    if not outfile:
        outfile   = f'{proc_home}/{jobj.tcoh_mask}'
...
def run_timeseries_rms(jobj, file, maskfile=None, rms_cutoff=None, deramp='no', figsize=[10,5]):
    proc_home = os.path.expanduser(jobj.proc_home)
    if not maskfile:
        maskfile = f'{proc_home}/{jobj.tcoh_mask}'
    if not rms_cutoff:
        iDict = readfile.read_template(jobj.template)
...
def run_plate_motion(jobj, vfile=None):
    om_cart = jobj.euler_cartesian
    iargs = ['--geom', jobj.geom_file, '--om-cart', *om_cart, '--velo', vfile]
    iargs = list(map(str,iargs))
    cmd   = 'plate_motion.py '+' '.join(iargs)
    print()
    print(cmd)
    plate_motion.main(iargs=iargs)
    return
...
def main(iargs=None):
    ...
    print('\n\n###################################################################')
    print(f'#### MintPy recipe | step: {inps.dosteps} | Start')
    print(f'#### Time: {t_string}')
    print('###################################################################\n')
    print(f'Run the follow stages in sequence: {inps.dosteps}')
    ...
    print(f'Current directory: {jobj.proc_home}')
    os.chdir(jobj.proc_home)
    ...
    if any(key in inps.dosteps for key in STEPS_SUPP['plot_velo']):
        ...
        inf    = os.path.expanduser(f'{jobj.velo_dir}/velocity')
    ...
    print('\n###################################################################')
    print(f'#### MintPy recipe | step: {inps.dosteps} | Normal finish.')
    print(f'#### Time: {t_string}')
