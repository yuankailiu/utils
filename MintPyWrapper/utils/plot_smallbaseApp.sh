...
log_file="${proc_home}/plot_SBApp.log"
...
if [ $plot_loaded_data -eq 1 ]; then
    ...
    test -f $file && h5ls $file/unwrapPhase      && $view $file unwrapPhase-      $opt --zero-mask --vlim -30 30 >> $log_file
...
## Loaded Ionosphere
if [ $plot_loaded_iono -eq 1 ]; then
    file=inputs/ionStack.h5
    opt='--noaxis --fontsize 8'
    #test -f $file && h5ls $file/unwrapPhase        && $view $file unwrapPhase-        $opt --zero-mask --wrap        >> $log_file
    test -f $file && h5ls $file/unwrapPhase        && $view $file unwrapPhase-        $opt --mask $water_mask --wrap >> $log_file
fi
...
## Plot ionosphere phase and TEC time series
...
## Plot SET map time series
opt1='--mask '$water_mask' --noaxis -u mm --ref-lalo 29.5463 36.0810'
if [ $plot_SET_ts -eq 1 ]; then
    # SET time-series
    file=inputs/SET.h5;                     test -f $file && $view $file $opt1 >> $log_file
fi
