...
def cmdLineParse():
    ...
    parser.add_argument('-dir', dest='dir', type=str, required=True,
            help = 'data directory containing all the date/pair folders')
    ...
    parser.add_argument('-file', dest='fname', type=str, required=True,
            help = 'file to plot')
    parser.add_argument('-band', dest='band', type=int, default=1,
            help = 'band of dataset. (default: %(default)s)')
...
'''
    ...
    files = sorted(glob.glob(os.path.join(inps.dir, '*', inps.fname) )) # (pair is -3)
    nfiles = len(files)
    for i in range(nfiles):
        ...
        cmd = 'mdx {} -s {} -ch{} -r4 -rtlr {} -CW -cmap Grey -P -workdir {}'.format(
            files[i],
            width,
            inps.band
            width*4,
