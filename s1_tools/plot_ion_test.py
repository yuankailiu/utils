...
'''
    ...
    files = sorted(glob.glob( os.path.join(inps.dir, '*_*', 'ion_cal/filt.ion') ))
    nfiles = len(files)
    for i in range(nfiles):
        pair = files[i].split('/')[-3]
        ...
        if not inps.overamp:
            ...
            cmd = 'mdx {} -s {} -amp -r4 -rtlr {} -CW -unw -r4 -rhdr {} -wrap 6.28 -addr -3.14 -cmap CMY -P -workdir {}'.format(
                files[i],
                width,
                width*4,
                width*4,
