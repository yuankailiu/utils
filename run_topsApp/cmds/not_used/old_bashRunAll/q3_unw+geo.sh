...
for ((i=0;i<${#insarpair[@]};i++)); do
    ...
    cmds/2_filt2unw.sh ${insarpair[i]}
...
pairstr=$(printf '%s\n' "${insarpair[@]}")
body=$(printf "Pairs has finished processing until geocode in job3:\n$pairstr")
printf "$body" | mail -s 'InSAR processing completed [job3 stage2+3]' ykliu@caltech.edu
