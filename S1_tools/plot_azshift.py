...
'''
    ...
    imgdir = 'img'
    os.mkdir(imgdir)
    files = sorted(glob.glob( os.path.join(inps.dir, '*-*', 'ion/ion_cal/azshift.ion') ))
    nfiles = len(files)
    for i in range(nfiles):
        ...
        #unit: cm
        azimuth_pixel_spacing = 14.06836 * 100.0

        cmd = "imageMath.py -e='a_0;a_1*{}' --a={} -o {} -t float -s BIL".format(
            azimuth_pixel_spacing,
            files[i],
            os.path.join(imgdir, 'azshift.ion')
            )
        runCmd(cmd)

        cmd = 'mdx {} -s {} -ch2 -r4 -rhdr {} -wrap 40 -addr -20 -cmap CMY -P -workdir {}'.format(
            os.path.join(imgdir, 'azshift.ion'),
            width,
            width*4,
            imgdir)
        ...
        runCmd(cmd)
        os.remove(os.path.join(imgdir, 'azshift.ion'))
        os.remove(os.path.join(imgdir, 'azshift.ion.vrt'))
        os.remove(os.path.join(imgdir, 'azshift.ion.xml'))
