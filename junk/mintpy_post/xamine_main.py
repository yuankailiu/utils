...
transec_swath_w = 180.0 * transec_swath_w / earth_radius / np.pi / step_deg
...
#%%
cmd =  'view.py velocity_out/velocity_RlP.h5 velocity --noverbose '
cmd += '--pts-file prof_tmp_pts.txt --pts-marker wo --pts-ms 5 '
cmd += '-m maskTempCoh.h5 -d inputs/geometryGeo.h5 '
cmd += '--alpha 0.5 --dem-nocontour --shade-exag 0.05 --figtitle velocty_RlP '
cmd += '--sub-lon 34 37.5 --sub-lat 27.5 31.7  --vlim -3 3 -u mm '
obj = viewer(cmd)
obj.configure()
obj.plot()

#%% Draw transections of the velocity map
...
plt.savefig(f'{pic_dir}/transects_err.png', dpi=150, bbox_inches='tight')
