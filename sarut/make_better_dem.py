...
dem_orig  = '{}'.format(jdic['dem_orig'])        # absolute path

if jdic['geom_file'].startswith('/'):   geo_file = '{}'.format(jdic['geom_file'])               # identify as absolute path
else:                                   geo_file = '{}/{}'.format(proc_home, jdic['geom_file']) # identidy as relative path under proc_home
if jdic['dem_out'].startswith('/'):     dem_out  = '{}'.format(jdic['dem_out'])                 # identify as absolute path
else:                                   dem_out  = '{}/{}'.format(proc_home, jdic['dem_out'])   # identidy as relative path under proc_home
...
# compute latitude and longitude min max (for gdalwarp -te, extend one more step at margins to get correct coverage)
i = 1
lon_min = float(atr['X_FIRST']) - float(atr['X_STEP']) * i
lon_max = float(atr['X_FIRST']) + float(atr['X_STEP']) * (int(atr['WIDTH']) +i)
lat_max = float(atr['Y_FIRST']) - float(atr['Y_STEP']) * i
lat_min = float(atr['Y_FIRST']) + float(atr['Y_STEP']) * (int(atr['LENGTH'])+i)
